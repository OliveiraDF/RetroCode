...
static AFX_EXTENSION_MODULE RetroMFCDLL = { FALSE, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RetroMFC.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RetroMFCDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RetroMFCDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de RetroMFC.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RetroMFCDLL);
	}
	return 1;   // ok
}
