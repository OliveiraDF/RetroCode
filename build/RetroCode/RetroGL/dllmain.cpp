...
static AFX_EXTENSION_MODULE RetroGLDLL = { FALSE, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RetroGL.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RetroGLDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RetroGLDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de RetroGL.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RetroGLDLL);
	}
	return 1;   // ok
}
